...
_TEAM_MODE=$(~/.gemini/antigravity/skills/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .gemini/antigravity/skills/ 2>/dev/null || true
if ! grep -qF '.gemini/antigravity/skills/' .gitignore 2>/dev/null; then
  echo '.gemini/antigravity/skills/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
