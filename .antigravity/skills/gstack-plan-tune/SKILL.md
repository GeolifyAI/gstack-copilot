...
7. **"Turn it off" / "disable"** → `~/.gemini/antigravity/skills/bin/gstack-config set question_tuning false`
8. **"Turn it on" / "enable"** → `~/.gemini/antigravity/skills/bin/gstack-config set question_tuning true`
...
1. Read the current state:
   ```bash
   _QT=$(~/.gemini/antigravity/skills/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
3. If A or B: enable:
   ```bash
   ~/.gemini/antigravity/skills/bin/gstack-config set question_tuning true
   ```
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.gemini/antigravity/skills/bin/gstack-developer-profile --read >/dev/null
...
```bash
~/.gemini/antigravity/skills/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ~/.gemini/antigravity/skills/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.gemini/antigravity/skills/bin/gstack-developer-profile --gap
```
...
```bash
~/.gemini/antigravity/skills/bin/gstack-question-preference --stats
eval "$(~/.gemini/antigravity/skills/bin/gstack-slug 2>/dev/null)"
_LOG="${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/question-log.jsonl"
[ -f "$_LOG" ] && echo "TOTAL_LOGGED: $(wc -l < "$_LOG" | tr -d ' ')" || echo "TOTAL_LOGGED: 0"
~/.gemini/antigravity/skills/bin/gstack-developer-profile --profile | bun -e "
